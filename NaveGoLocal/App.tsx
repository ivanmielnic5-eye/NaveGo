...
import { StyleSheet, Text, View, TouchableOpacity, Alert, ActivityIndicator, InteractionManager, useWindowDimensions } from 'react-native';
...
function AppContent() {
  ...
  useEffect(() => {
    if (permissionState === 'granted' && mapReady) {
  ...
  useEffect(() => {
    const updateNavBar = async () => {
      if (isLandscape) {
        ...
        await NavigationBar.setBackgroundColorAsync('#030817');
  ...
  const gpsStateText = tracker.isTracking ? 'GPS OK' : 'BUSCANDO...';
  const gpsColor = tracker.isTracking ? colors.success : colors.warning;
  ...
  const navConfig = {
    ...
    NO_DISPONIBLE: { icon: 'location-off' as const, color: colors.textSecondary, label: 'GNSS NO DISPONIBLE' },
    GNSS_PERDIDO: { icon: 'location-off' as const, color: colors.warning, label: 'GNSS PERDIDO' },
      ...
      <View style={styles.landscapeContainer}>
        ...
        >
          {routePoints.length > 1 && (
            <Polyline coordinates={routePoints} strokeWidth={2} strokeColor={colors.navigateCyan} />
          )}
        </MapView>
        {lastPoint && (
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>
          NAVEGO <Text style={styles.headerAccent}>// HUD</Text>
        </Text>
        <View style={[glass.statusChip, { borderColor: gpsColor, backgroundColor: tracker.isTracking ? 'rgba(53,211,154,0.10)' : 'rgba(255,181,46,0.10)' }]}>
          <Text style={[styles.headerStatus, { color: gpsColor }]}>{gpsStateText}</Text>
        </View>
      </View>
      ...
      <View style={styles.mapWrapper}>
        ...
        {lastPoint && (
          <TouchableOpacity style={[glass.mapControl, styles.centerButton]} onPress={handleCenterMap}>
            <Text style={styles.centerButtonText}>CENTRAR</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={glass.substrate}>
        <View style={styles.row}>
          <TelemetryTile label="SOG (NUDOS)" value={sogKnots} />
          <TelemetryTile label="COG (RUMBO)" value={`${cogDeg}°`} accent />
        </View>
          ...
          <View style={styles.distanceCompactContent}>
            <View style={styles.distanceHeader}>
              ...
              <Text style={styles.distanceValue}>
                {distanceKm} <Text style={styles.distanceUnit}>KM</Text>
              </Text>
            </View>
            ...
            <View style={styles.statusRowCompact}>
              ...
              <Text style={[styles.recordingText, { color: tracker.isPaused ? colors.warning : colors.navigateCyan }]}>
                {tracker.isPaused ? 'II PAUSADO' : '● GRABANDO'}
              </Text>
            </View>

            {tracker.activeHazards && tracker.activeHazards.length > 0 && (
              <View style={styles.alertBannerCompact}>
                <Text style={styles.alertTextCompact}>
                  {tracker.activeHazards[0].confidence === 'CONFIRMADO' ? '⚠️' : '◌'}{' '}
                  {tracker.activeHazards[0].name} a {Math.round(tracker.activeHazards[0].distance)} m
                </Text>
                <Text style={styles.alertSubTextCompact}>
                  {tracker.activeHazards[0].confidence === 'CONFIRMADO' ? 'CONFIRMADO' : 'ÚLTIMA INFORMACIÓN'} · hace{' '}
                  {tracker.activeHazards[0].lastReportedAt
                    ? Math.floor((Date.now() - tracker.activeHazards[0].lastReportedAt) / 60000)
                    : 0}{' '}
                  min
                </Text>
              </View>
        ...
        {/* DOCK FLOTANTE CORREGIDO */}
        <View style={styles.dockBar}>
          {floatingActions.map((item) => (
            <TouchableOpacity key={item.key} style={styles.dockItemTouchable}>
              <Ionicons name={item.icon} size={22} color={colors.navigateCyan} />
        ...
        <View style={styles.row}>
          <TouchableOpacity
            style={[glass.actionControl, { flex: 1, backgroundColor: tracker.isPaused ? 'rgba(53,211,154,0.14)' : 'rgba(255,181,46,0.12)' }]}
            onPress={handleToggleRecord}
          >
            <Text style={[styles.actionButtonText, { color: tracker.isPaused ? colors.success : colors.warning }]}>
              {tracker.isPaused ? 'REANUDAR' : 'PAUSAR'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[glass.actionControl, { flex: 1 }]}
            onPress={handleReset}
          >
        ...
        <View style={styles.finalContainer}>
          <TouchableOpacity
            style={[glass.actionControl, { paddingHorizontal: 24, backgroundColor: 'rgba(5,25,42,0.55)', borderColor: colors.borderSubtle }]}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: colors.background, paddingHorizontal: 16, paddingTop: 4, paddingBottom: 12, justifyContent: 'space-between' },
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 6, marginBottom: 8 },
  headerTitle: { fontSize: 18, fontWeight: 'bold', color: colors.textPrimary, letterSpacing: 1.2 },
  ...
  mapWrapper: { height: 190, borderRadius: radii.md, overflow: 'hidden', marginBottom: 10, borderWidth: 1, borderColor: colors.glassBorderStrong },
  map: { ...StyleSheet.absoluteFillObject },
  mapLoading: { ...StyleSheet.absoluteFillObject, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(3, 7, 23, 0.7)' },
  centerButton: { position: 'absolute', top: 60, right: 8 },
  ...
  distanceCardCompact: { padding: 0, marginBottom: 10 },
  distanceCompactContent: { padding: spacing.md },
  distanceHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  label: { ...fonts.label },
  value: { fontSize: 34, fontWeight: 'bold', letterSpacing: -1 },
  unit: { fontSize: 18, color: colors.textSecondary },
  distanceValue: { fontSize: 26, fontWeight: 'bold', letterSpacing: -0.8, color: colors.navigateCyan },
  ...
  controlText: { fontSize: 13, fontWeight: 'bold', letterSpacing: 0.8 },
  finalContainer: { alignItems: 'center', marginBottom: 8 },
  landscapeContainer: { flex: 1, backgroundColor: colors.background },
  landscapeCenterButton: { position: 'absolute', top: 60, right: 12 },
  landscapeGpsBadge: { position: 'absolute', top: 12, left: 12 },
});