...
function AppContent() {
  ...
  // ===== AMARRE: ya no se inicia tracking automáticamente =====
  // useEffect de inicio automático eliminado. El tracking se inicia con el botón INICIAR DERROTA.
    ...
    return (
      <View style={styles.landscapeContainer}>
        ...
        <MapView
          ...
          camera={courseUpActive ? {
            center: {
              latitude: lastPoint?.latitude ?? initialPosition.latitude,
              longitude: lastPoint?.longitude ?? initialPosition.longitude,
            },
            heading: tracker.smoothedCog ?? 0,
            pitch: 0,
            zoom: 17,
          } : undefined}
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.mapWrapper}>
        {!mapReady && <View style={styles.mapLoading}><ActivityIndicator size="large" color={colors.navigateCyan} /></View>}
        <MapView
          ref={mapRef}
          style={styles.map}
          initialRegion={{
        ...
        <View style={styles.row}>
          ...
          <View style={[glass.telemetryTile, { flex: 1, padding: 0 }]}>
            <View style={styles.cogTileContent}>
              ...
              <Text style={[styles.value, { color: colors.navigateCyan }]}>{`${cogDeg}°`}</Text>
              ...
              <Text style={styles.cogSubLabel}>RUMBO SOBRE FONDO</Text>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  primaryAction: { backgroundColor: colors.glassSuccess, borderWidth: 1, borderColor: colors.success, borderRadius: radii.sm, paddingVertical: 14, alignItems: "center" },
  primaryActionText: { color: colors.success, fontWeight: "bold", fontSize: 14, letterSpacing: 1 },
  actionHalf: { flex: 1, borderWidth: 1, borderColor: colors.borderSubtle, borderRadius: radii.sm, paddingVertical: 12, alignItems: "center" },
  finalizeAction: { borderWidth: 1, borderColor: colors.borderSubtle, borderRadius: radii.sm, paddingVertical: 12, alignItems: "center", marginTop: 8, backgroundColor: "rgba(5,25,42,0.55)" },
  finalizeActionText: { color: colors.textPrimary, fontWeight: "bold", fontSize: 13, letterSpacing: 0.8 },
});
